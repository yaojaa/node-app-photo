...
(function () {
    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            uploader = _this.uploader = WebUploader.create({
                ...
                server: editor.getOpt('imageActionName'),
                // fileVal: editor.getOpt('imageFieldName'),
                duplicate: true,
                formData:{
                    token:editor.getOpt('token')
                  },
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                var $file = $('#' + file.id);
                try {
                    var responseText = ret,
                        json = {
                            original:responseText.hash,
                            name:responseText.key,
                            url:responseText.key,
                            state:"SUCCESS"
                        }

                    if (responseText.hash) {
                        _this.imageList.push(json);
                        $file.append('<span class="success"></span>');
                        saveImgToLocal(json.url)
                    } else {
    ...
    /* 最近图片*/
 var uploadList=getImgFromLocal() ;
 function getImgFromLocal(){
    var ls = window.localStorage.getItem('UPLOAD_IMG_LIST_STOR')
    if(ls){
      ls = JSON.parse(ls)
    }
  return  ls  || []
 ...
 function saveImgToLocal(url){
    uploadList.push({url:editor.getOpt('imageUrlPrefix')+url,mtime:new Date().getTime()})
    uploadListToString = JSON.stringify(uploadList)
    window.localStorage.setItem('UPLOAD_IMG_LIST_STOR',uploadListToString)
 }
        ...
        pushData: function (list) {
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    img.width = 113;
                    //http://img.fengimage.com/FmQaaXx53P56vLYfLPMrQow-19lb?imageView2/1/w/113/h/113
                    img.setAttribute('src', list[i].url+'?imageView2/1/w/113/h/113' + '&noCache=' + (+new Date()).toString(36) );
